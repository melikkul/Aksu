...
| ⚡ **CPU training (re-timing pending)** | No GPU required — frozen BERTurk + 1M param reranker |
...
|--------|-------------|----------|----------|
| **Aksu disambiguator** | **pending re-timing [^hw]** | **TRUBA Orfoz (Intel Xeon Platinum 8362, CPU-only)** | **98.3%** [^ensemble] |
...
| Training wall-clock | `scripts/truba/submit_train_disambiguator.sh` | `models/v6_retimed/training_log.json` | ⏳ pending Orfoz run (resubmitted) |
...
|------|----------|-----------|
| Disambiguation training (1 seed) | CPU only — TRUBA Orfoz (Intel Xeon Platinum 8362) | pending re-timing |
